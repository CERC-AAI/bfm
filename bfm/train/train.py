...
class SeedableNumInstanceSampler(ExpectedNumInstanceSampler):
    def __init__(
        self,
        num_instances: float,
        min_instances: int = 0,
...
def load_model(
    ...
    if resume_from_checkpoint:
        log_on_main(f"Using pretrained initialization from {checkpoint_dir}", logger)
        ...
        config = AutoConfig.from_pretrained(
            model_id, cache_dir=checkpoint_dir, local_files_only=False
        )
        ...
        print("checkpoint_dir 2", checkpoint_dir)
...
class PseudoShuffledIterableDataset(IterableDataset):
    ...
    def __init__(
        self, base_dataset, shuffle_buffer_length: int = 100, seed: int = 0
    ) -> None:
        super().__init__()
...
class ShuffleMixin:
    ...
    def shuffle(self, shuffle_buffer_length: int = 100, seed: int = 0):
        return PseudoShuffledIterableDataset(self, shuffle_buffer_length, seed)
...
class ChronosDataset(IterableDataset, ShuffleMixin):
    ...
    def _create_instance_splitter(self, mode: str):
        ...
        instance_sampler = {
            "training": SeedableNumInstanceSampler(
                num_instances=1.0,
                min_instances=1,
...
def main(
    ...
    validation_data_paths: str,
    dataset: str,
    project_dir: str,
    experiment_name: str = "test",
    wandb_mode: str = "offline",
    n_gpus: int = 8,
    wandb_entity: str = "brain_fomo",
    wandb_project: str = "brain_fomo",
    context_length: int = 512,
    prediction_length: int = 64,
    min_past: int = 64,
    ...
    assert isinstance(validation_data_paths, list)
    ...
    directory_path = (
        project_dir + "Experiments/" + experiment_name + "_" + dataset + "/" + str(seed)
    )
    if is_main_process():
        if not os.path.exists(directory_path):
            os.makedirs(directory_path)
    primary_checkpoint_dir = directory_path
    secondary_checkpoint_dir = "bfm/model_weights/"
    os.environ["HF_HOME"] = secondary_checkpoint_dir

    # Determine the checkpoint directory to use
    if get_latest_checkpoint(primary_checkpoint_dir, n_gpus):
        resume_from_checkpoint = True
        checkpoint_dir = get_latest_checkpoint(primary_checkpoint_dir, n_gpus)
    else:
        resume_from_checkpoint = False
        checkpoint_dir = secondary_checkpoint_dir
    ...
    if is_main_process():
        ...
        print("full_experiment_name", full_experiment_name)
        wandb.init(
            entity=wandb_entity,
            project=wandb_project,
            name=full_experiment_name,
            mode=wandb_mode,
            dir=os.path.join(primary_checkpoint_dir),
    ...
    training_data_paths = get_arrow_files(training_data_paths)
    validation_data_paths = get_arrow_files(validation_data_paths)
    ...
    train_datasets = [
        Filter(
            ...
            FileDataset(path=Path(data_path), freq="10ms"),
        )
    ...
    shuffled_train_dataset = ChronosDataset(
        ...
        probabilities=[1.0 / len(training_data_paths)] * len(training_data_paths),
        ...
        mode="training",
        seed=seed,
    ).shuffle(shuffle_buffer_length=shuffle_buffer_length, seed=seed)

    val_dataset = ChronosDataset(
        datasets=val_datasets,
        probabilities=[1.0 / len(validation_data_paths)] * len(validation_data_paths),
        tokenizer=chronos_config.create_tokenizer(),
        context_length=context_length,
        prediction_length=prediction_length,
        min_past=min_past,
    ...
    training_args = TrainingArguments(
        output_dir=primary_checkpoint_dir,
        ...
        optim=optim,
        logging_dir=primary_checkpoint_dir + "/logs",
    ...
    if resume_from_checkpoint:
        ...
        print("Checkpoint directory does not exist. Starting fresh training.")
        result = trainer.train()

    with open(primary_checkpoint_dir + "/logs/logs.json", "w") as f:
        json.dump(trainer.state.log_history, f)
    if is_main_process():
        model.save_pretrained(
            Path(primary_checkpoint_dir + "/output/checkpoint-final"),
            safe_serialization=False,
        )

    print("model saved at ", primary_checkpoint_dir)

    # wandb.finish()


if __name__ == "__main__":
    print("number of visible devices", torch.cuda.device_count())
